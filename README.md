...
- Set variables (NET=models/checkmodel2phase)(DATASET=data/dataset) 

- Go into the Test folder and select an image

- Run the command to process the image (imagenet.py --model=$NET/resnet18.onnx --input_blob=input_0 --output_blob=output_0 --labels=$DATASET/labels.txt $DATASET/test/<fractured/not_fractured>/<IMAGE_NAME>jpg <RESULT_NAME>.jpg)
